...
namespace core
{
	...
	std::vector<std::size_t> MeshCloud::get_vertices_from_face(std::size_t fi,bool is_triangle_mesh)
	{
	...
	bool MeshCloud::MPF_Query(const RowVector3d& qpoint, double max_distance,RowVector3d& closest_point,std::size_t& closest_face,std::size_t& closest_vertex)
	{
		...
		for(std::size_t k = 0; k < triangle_prox.size(); ++k)
		{
			...
			std::vector<std::size_t> triangle_points = get_vertices_from_face(triangle_prox[k],1);
